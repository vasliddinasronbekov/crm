...
Run with: python manage.py create_english_quizzes
"""
...
class Command(BaseCommand):
    help = 'Create comprehensive English quizzes for all levels'

    def handle(self, *args, **options):
        self.stdout.write("Starting English quiz creation...\n")
        ...
        try:
            ...
            if not english_course:
                ...
                english_course = Course.objects.create(
                    ...
                    price=50000000,  # 500,000 UZS in tiyins
                    duration_months=6
                )
                self.stdout.write(self.style.SUCCESS(f"✓ Created English course: {english_course.name}"))
            else:
                self.stdout.write(self.style.SUCCESS(f"✓ Using existing course: {english_course.name}"))
        ...
        medium_quiz_data = {
            ...
            'questions': [
                ...
                {
                    'text': 'Which sentence demonstrates correct passive voice:',
                    'type': 'multiple_choice',
                    'points': 2,
                    'options': [
                ...
                {
                    'text': 'Choose the correct preposition: I am interested ___ learning English.',
                    'type': 'multiple_choice',
                    'points': 2,
                    'options': [
                    ...
                    'explanation': 'We say "interested in" (not at/on/for) when talking about hobbies or subjects.'
                },
            ]
        }
        ...
        for level, quiz_data in quiz_datasets:
            try:
                ...
                if existing_quiz:
                    self.stdout.write(self.style.WARNING(f"⚠ {level} quiz already exists: {quiz_data['title']}"))
                    continue
                ...
                self.stdout.write(self.style.SUCCESS(f"✓ Created {level} quiz: {quiz.title} ({len(questions_data)} questions)"))
                created_count += 1

            except Exception as e:
                self.stdout.write(self.style.ERROR(f"✗ Error creating {level} quiz: {e}"))

        self.stdout.write("\n" + "=" * 60)
        self.stdout.write(self.style.SUCCESS(f"Quiz creation complete! Created {created_count} new quizzes."))
        self.stdout.write(self.style.SUCCESS(f"Total quizzes in database: {Quiz.objects.filter(course=english_course).count()}"))
