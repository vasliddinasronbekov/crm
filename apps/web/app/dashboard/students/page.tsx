...
import {
  ...
  User, Calendar, MapPin, Award, BookOpen, TrendingUp,
  Filter, Download, Upload, UserPlus, Eye, MoreVertical
} from 'lucide-react'
...
export default function StudentsPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'with_email' | 'with_phone'>('all')

  const { data: studentsData, isLoading, error, isFetching } = useStudents({
    page,
    limit,
    search: debouncedSearchQuery,
    has_email: filterStatus === 'with_email' ? true : undefined,
    has_phone: filterStatus === 'with_phone' ? true : undefined,
  })
  ...
  const stats = {
    ...
    withEmail: students.filter((s: Student) => s.email).length,
    withPhone: students.filter((s: Student) => s.phone).length,
    recentlyAdded: students.filter((s: Student) => {
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-background p-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-text-secondary">Manage student accounts and information</p>
        </div>
